...
class MultiTask_StoryCompletionModel(nn.Module):
    def __init__(self, 
                 SentenceEncoder, device, ContextEncoder,
        ...
        self.decoder = BertForMaskedLM.from_pretrained('bert-base-uncased', is_decoder=True, output_hidden_states=True)
        
        self.mpp_classifier = nn.Linear(768, 5)        
        # self.mpp_classifier = MLP(self.decoder.config.hidden_size, 768, 5, 0.5)
        
        self.device = device
        self.no_contextencoder_before_languagemodel = no_contextencoder_before_languagemodel
    ...
    def forward(self, story, story_mask, target=None, max_length=32, trg_start_id=101):
        ...
        # Inference
            ...
            for t in range(1, max_length):            
                decoder_outputs = self.decoder(input_ids=generated, encoder_hidden_states=hidden)
                ...
                if t == 1:
                    sc_outputs = torch.zeros(batch_size, 1, last_predictions.shape[-1]).to(self.device)                                       
    ...
    def generate(self, story, story_mask, tokenizer, max_length=32, trg_start_id=101):
        with torch.no_grad():
            generated_sentences = []
            generated, _, mpp_outputs = self.forward(story, story_mask, max_length=max_length)
            
            # generated = generated[1:].transpose(0, 1)
            
            for output_ind in generated:
                try:
                    output_decoded = tokenizer.decode(output_ind.cpu().numpy(), skip_special_tokens=True)
...
def train(model, iterator, optimizer, sc_criterion, mpp_criterion, clip, scheduler, loss_weight):
    ...
    for i, batch in enumerate(tqdm(iterator, desc="Iteration")):        
        ...
        epoch_mpp_acc += (mpp_outputs.argmax(1) == cls).sum().item() / (batch_size + .0)
        
    return epoch_loss / len(iterator), epoch_mpp_acc / len(iterator)


def evaluate(model, iterator, sc_criterion, mpp_criterion, loss_weight, tb_valid_text=0):
        ...
        for i, batch in enumerate(tqdm(iterator, desc="Iteration")):        
            ...
            if tb_valid_text != 0 and i == 0:
                ...
                for j, output_ind in enumerate(generated[:tb_valid_text]):
                    try:
                        output_decoded = tokenizer.decode(output_ind.cpu().numpy(), skip_special_tokens=True)
                        tb_writer.add_text(f"example {j}", output_decoded, global_step=epoch+1)
                    except:
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(iterator, desc="Iteration")):                
            ...
            for i in range(batch_size):
                ...
                predicted_story = " ".join(np.insert(story[i], predicted[i], sc_generated[i])).lower()
                # gt_missing_story = " ".join(np.insert(story[i], cls[i], missing_sentence))
                gt_story = " ".join(np.insert(story[i], cls[i], original_sentence[i])).lower()

                result_to_show.append([input_story, predicted[i], sc_generated[i], predicted_story, cls[i], original_sentence[i], gt_story])
      
    show_result_df = pd.DataFrame(result_to_show, 
                                  columns=["input", "pred_missing_id (0_indexed)", "generated_sentence", "pred_story", 
                                           "gt_missing_id (0_indexed)", "gt_sentence", "gt_story"])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task', type=str, default='completion',
                        help="task to solve: completion, ending, ending_mini")    
    ...
    parser.add_argument("--no_contextencoder_lm", action="store_true", help="Not using Context Encoder before BERT decoder LM.")

    parser.add_argument("--save_every_epoch", action="store_true", help="Save the model of every epoch.")    

    parser.add_argument("--tb_valid_text", default=0, type=int, help="The number of example texts for tensorboard every epoch.")    
    ...
    if args.task == "completion":
        ...
        train_dataset = ROCStoriesDataset(data_path = "../data/rocstories_for_storyendinggeneration_train.csv")
        val_dataset = ROCStoriesDataset(data_path = "../data/rocstories_for_storyendinggeneration_val.csv")            
    
    sentbertmodel = SentenceTransformer('bert-base-nli-mean-tokens')
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
            
    # --- model --- 
    model = MultiTask_StoryCompletionModel(SentenceEncoder=sentbertmodel, 
                                           device=device,
    ...
    with mlflow.start_run():
        ...
        for epoch in trange(N_EPOCHS, desc="Epoch"):
            ...
            train_loss, train_mpp_acc = train(model, train_iterator, optimizer, sc_criterion, mpp_criterion, 
                                              CLIP, scheduler=scheduler, 
                                              loss_weight=args.loss_weight)
            valid_loss, valid_mpp_acc = evaluate(model, valid_iterator, sc_criterion, mpp_criterion, 
                                                 loss_weight=args.loss_weight,
            ...
            if valid_mpp_acc > best_valid_mpp_acc:
                ...
                torch.save(model.state_dict(), os.path.join(args.output_dir, 'best_mpp_acc_model.pt'))            
            
            # save each epoch model
            if args.save_every_epoch:
                torch.save(model.state_dict(), os.path.join(args.output_dir, 'epoch_{:0=3}_model.pt'.format(epoch+1)))            

            print(f'Epoch: {epoch+1:02} | Time: {epoch_mins}m {epoch_secs}s')
            # print(f'lr: {scheduler.get_lr()[0]}')
            print(f'\tlr: {scheduler.get_last_lr()[0]}')        
            print(f'\tTrain Loss: {train_loss:.3f} | Train MPP Accuracy: {train_mpp_acc * 100:.1f}%')
            print(f'\t Val. Loss: {valid_loss:.3f} |  Val. MPP Accuracy: {valid_mpp_acc * 100:.1f}%')
